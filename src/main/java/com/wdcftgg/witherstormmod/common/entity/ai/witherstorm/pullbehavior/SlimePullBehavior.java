...
public final class SlimePullBehavior extends WitherStormPullBehavior<EntitySlime> {
    ...
    @Override
    public Vec3d pullEntity(EntitySlime entity, WitherStormEntity storm, Vec3d absorptionPoint,
                            Vec3d defaultVelocity, double defaultSpeed) {
        if (entity.getPositionVector().distanceTo(absorptionPoint) > storm.width * 1.5D) {
    ...
    @Override
    public double getSpeed(EntitySlime entity, WitherStormEntity storm, Vec3d absorptionPoint) {
