...
public class SickenedParrotModel extends ModelBase
{
    ModelRenderer body;
    ModelRenderer tail;
    ...
    private SickenedParrotModel.State state = SickenedParrotModel.State.STANDING;

    public SickenedParrotModel()
    {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn)
    {
        ...
        if (this.state != SickenedParrotModel.State.FLYING)
        {
            if (this.state == SickenedParrotModel.State.SITTING)
            {
                return;
            }

            if (this.state == SickenedParrotModel.State.PARTY)
            {
    ...
    public void setLivingAnimations(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTickTime)
    {
        ...
        if (entitylivingbaseIn instanceof SickenedEntities.SickenedParrotEntity)
        {
            SickenedEntities.SickenedParrotEntity entityparrot = (SickenedEntities.SickenedParrotEntity) entitylivingbaseIn;

            if (entityparrot.isSickenedPartying())
            {
                this.legLeft.rotateAngleZ = -0.34906584F;
                this.legRight.rotateAngleZ = 0.34906584F;
                this.state = SickenedParrotModel.State.PARTY;
                return;
            }

            if (entityparrot.isSickenedSitting())
            {
                float f = 1.9F;
                ...
                this.state = SickenedParrotModel.State.SITTING;
            }
            else if (entityparrot.isSickenedFlying())
            {
                this.legLeft.rotateAngleX += ((float)Math.PI * 2F / 9F);
                this.legRight.rotateAngleX += ((float)Math.PI * 2F / 9F);
                this.state = SickenedParrotModel.State.FLYING;
            }
            else
            {
                this.state = SickenedParrotModel.State.STANDING;
            }
